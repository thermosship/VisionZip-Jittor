...
> 当前进度：第一阶段核心算法已完成 FP32 对齐；下一步进行核心性能测试和真实视觉特征验证。
...
1. 运行并整理核心性能测试；
2. 从官方 CLIP 导出真实图像的 Hidden State、Attention 和 Key Metric；
3. 使用 Jittor 对真实视觉特征进行对齐；
4. 接入多模态 Projector 和冻结 LLM；
5. 实现只训练 Projector 的高效微调；
6. 对齐 Loss、效果、速度、显存和可视化；
7. 整理最终 README、PPT 和视频素材。
